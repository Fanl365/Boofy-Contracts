...
import vaultV7 from "../../artifacts/contracts/BIFI/vaults/BoofyVaultV7.sol/BoofyVaultV7.json";
import vaultV7Factory from "../../artifacts/contracts/BIFI/vaults/BoofyVaultV7Factory.sol/BoofyVaultV7Factory.json";
...
const {
  platforms: { beethovenX, boofyfinance },
  tokens: {
    BAL: { address: BAL },
...
const strategyParams = {
  ...
  keeper: boofyfinance.keeper,
  boofyFeeRecipient: boofyfinance.boofyFeeRecipient,
  boofyFeeConfig: boofyfinance.boofyFeeConfig,
  isBeets: true,
  boofyVaultProxy: "0xA6D3769faC465FC0415e7E9F16dcdC96B83C240B",  //boofyfinance.vaultProxy,
...
async function main() {
  ...
  const factory = await ethers.getContractAt(vaultV7Factory.abi, strategyParams.boofyVaultProxy);
  ...
  vaultInitTx = await vaultContract.transferOwnership(boofyfinance.vaultOwner);
  vaultInitTx = await vaultInitTx.wait()
  vaultInitTx.status === 1
  ...
  const strategyConstructorArguments = [
    ...
    strategyParams.boofyFeeRecipient,
    strategyParams.boofyFeeConfig],
  ];
