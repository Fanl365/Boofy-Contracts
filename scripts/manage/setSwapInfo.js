...
import swapperAbi from "../../artifacts/contracts/BIFI/infra/BoofySwapper.sol/BoofySwapper.json";
...
const boofyfinanceSwapper = boofyfinance.boofySwapper;

const uniswapV3Router = "0x2626664c2603336E57B271c5C0b26F421741e481";
const uniswapV4Router = boofyfinance.boofyUniV4Swapper;
...
const multihopRouter = boofyfinance.boofyMultiHopSwapper;
...
async function balancer() {
  ...
  const swapSteps = [];
  const assets = [];
  const funds = [boofyfinanceSwapper, false, boofyfinanceSwapper, false];
...
async function solidly() {
  ...
  const txData = await router.populateTransaction.swapExactTokensForTokens(0, 0, config.solidly.path, boofyfinanceSwapper, uint256Max);
  const amountIndex = 4;
  const minIndex = 36;
  const minAmountSign = 0;
...
async function traderJoe() {
  ...
  const txData = await router.populateTransaction.swapExactTokensForTokens(0, 0, path, boofyfinanceSwapper, uint256Max);
  const amountIndex = 4;
  const minIndex = 36;
...
async function syncswap() {
  ...
  let data = ethers.utils.defaultAbiCoder.encode(
    ...
    [config.syncswap.route[0], boofyfinanceSwapper, 2]
  );
...
async function setSwapInfo(fromToken, toToken, swapInfo) {
  ...
  const swapper = await ethers.getContractAt(swapperAbi.abi, boofyfinanceSwapper, keeper);
