...
contract BoofyFeeConfigurator is OwnableUpgradeable {

    struct FeeCategory {
        uint256 total;      // total fee charged on each harvest
        uint256 boofy;      // split of total fee going to boofy fee batcher
    ...
    function getFeeCategory(uint256 _id, bool _adjust) public view returns (FeeCategory memory fees) {
        ...
        if (_adjust) {
            ...
            fees.boofy = fees.boofy * _totalFee / DIVISOR;
    ...
    function setFeeCategory(
        ...
        uint256 boofy = DIVISOR - _call - _strategist;

        FeeCategory memory category = FeeCategory(_total, boofy, _call, _strategist, _label, _active);
        feeCategory[_id] = category;
        emit SetFeeCategory(_id, _total, boofy, _call, _strategist, _label, _active);
    }
