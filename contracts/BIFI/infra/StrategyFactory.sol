...
// Minimal proxy pattern for creating new Boofy strategies
contract StrategyFactory is Ownable {
    ...
    /// @notice The boofy fee recipient
    address public boofyFeeRecipient;

    /// @notice The boofy fee config
    address public boofyFeeConfig;
    ...
    event SetBoofyFeeRecipient(address boofyFeeRecipient);
    event SetBoofyFeeConfig(address boofyFeeConfig);
    ...
    constructor(
        address _native,
        address _keeper,
        address _boofyFeeRecipient,
        address _boofyFeeConfig
    ) Ownable(msg.sender) {
        native = _native;
        keeper = _keeper;
        boofyFeeRecipient = _boofyFeeRecipient;
        boofyFeeConfig = _boofyFeeConfig;
    }

    /** @notice Creates a new Boofy Strategy as a proxy of the template instance
    * @param _strategyName The name of the strategy
    * @return A reference to the new proxied Boofy Strategy
   */
    function createStrategy(string calldata _strategyName) external returns (address) {

        // Create a new Boofy Strategy as a proxy of the template instance
   ...
   */
    ...
    /**
     * @notice set the boofy fee recipient address
   * @param _boofyFeeRecipient The new boofy fee recipient address
   */
    function setBoofyFeeRecipient(address _boofyFeeRecipient) external onlyOwner {
        boofyFeeRecipient = _boofyFeeRecipient;
        emit SetBoofyFeeRecipient(_boofyFeeRecipient);
    }

    /**
     * @notice set the boofy fee config address
   * @param _boofyFeeConfig The new boofy fee config address
   */
    function setBoofyFeeConfig(address _boofyFeeConfig) external onlyOwner {
        boofyFeeConfig = _boofyFeeConfig;
        emit SetBoofyFeeConfig(_boofyFeeConfig);
    }
