...
import { BoofyOracleHelper, IBoofyOracle, BoofyOracleErrors } from "./BoofyOracleHelper.sol";

/// @title Boofy Oracle for Algebra Integral
/// @notice On-chain oracle using Algebra
contract BoofyOracleAlgebraIntegral {
    ...
    function getPrice(bytes calldata _data) external returns (uint256 price, bool success) {
        ...
        price = BoofyOracleHelper.priceFromBaseToken(
    ...
    function validateData(bytes calldata _data) external view {
        ...
        if (tokens.length != pools.length + 1 || tokens.length != twapPeriods.length + 1) {
            revert BoofyOracleErrors.ArrayLength();
        }
        
        uint256 basePrice = IBoofyOracle(msg.sender).getPrice(tokens[0]);
        if (basePrice == 0) revert BoofyOracleErrors.NoBasePrice(tokens[0]);
        ...
        for (uint i; i < poolLength;) {
            ...
            if (fromToken != token0 && fromToken != token1) {
                revert BoofyOracleErrors.TokenNotInPair(fromToken, pool);
            }
            if (toToken != token0 && toToken != token1) {
                revert BoofyOracleErrors.TokenNotInPair(toToken, pool);
            }
            unchecked { ++i; }
        }
    }
