...
/// @title BoofyOracleERC4626
/// @author Beefy, kexley
/// @notice Oracle for ERC4626 vaults
contract BoofyOracleERC4626 {
    ...
    function getPrice(bytes calldata _data) external returns (uint256 price, bool success) {
        ...
        price = BoofyOracleHelper.priceFromBaseToken(msg.sender, vault, asset, amountOut);
        return (price, true);
    }
    ...
    function validateData(bytes calldata _data) external view {
        ...
        address asset = IERC4626(vault).asset();
        uint amountOut = IERC4626(vault).convertToShares(10 ** IERC4626(vault).decimals());
        if (vault == address(0) || asset == address(0) || amountOut == 0) revert BoofyOracleErrors.NoAnswer();
        if (IBoofyOracle(msg.sender).getPrice(asset) == 0) revert BoofyOracleErrors.NoBasePrice(asset);
    }
}