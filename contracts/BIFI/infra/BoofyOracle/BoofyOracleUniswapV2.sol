...
/// @title Boofy Oracle for UniswapV2
/// @author Beefy, @kexley
    ...
    function validateData(bytes calldata _data) external view {
        ...
        for (uint i; i < pairLength;) {
            ...
            if (fromToken != token0 && fromToken != token1) {
                revert BoofyOracleErrors.TokenNotInPair(fromToken, pair);
            }
            if (toToken != token0 && toToken != token1) {
                revert BoofyOracleErrors.TokenNotInPair(toToken, pair);
            }
            unchecked { ++i; }
        }
    }
