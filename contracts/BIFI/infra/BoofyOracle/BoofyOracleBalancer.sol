...
/// @title Boofy Oracle for Balancer
/// @author Beefy, @weso
/// @notice On-chain oracle using Balancer
contract BoofyOracleBalancer {
    ...
    function validateData(bytes calldata _data) external view {
        ...
        uint256 basePrice = IBoofyOracle(msg.sender).getPrice(assets[0]);
        if (basePrice == 0) revert BoofyOracleErrors.NoBasePrice(assets[0]);
