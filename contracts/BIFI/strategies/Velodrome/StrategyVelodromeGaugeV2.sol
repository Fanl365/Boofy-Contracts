...
import "../../interfaces/boofy/IBoofySwapper.sol";
...
contract StrategyVelodromeGaugeV2 is StratFeeManagerInitializable {
    ...
    event ChargedFees(uint256 callFees, uint256 boofyFees, uint256 strategistFees);
    event SetBoofySwapper(address newSwapper);

    function initialize(
        address _want,
        address _gauge,
        address _boofySwapper,
    ...
    function chargeFees(address callFeeRecipient) internal {
        ...
        (address router,,,,) = IBoofySwapper(boofySwapper).swapInfo(output, native);
        if (router != address(0)) {
            IBoofySwapper(boofySwapper).swap(output, native, toNative);
        } else {
    ...
    function addLiquidity() internal {
        ...
        if (lpToken0 != output) {
            (address router,,,,) = IBoofySwapper(boofySwapper).swapInfo(output, lpToken0);
            if (router != address(0)) {
                IBoofySwapper(boofySwapper).swap(output, lpToken0, lp0Amt);
            } else {
        ...
        if (lpToken1 != output) {
            (address router,,,,) = IBoofySwapper(boofySwapper).swapInfo(output, lpToken1);
            if (router != address(0)) {
                IBoofySwapper(boofySwapper).swap(output, lpToken1, lp1Amt);
            } else {
    ...
    function _giveAllowances() internal {
        ...
        IERC20(output).safeApprove(boofySwapper, type(uint).max);
    ...
    function _removeAllowances() internal {
        ...
        IERC20(output).safeApprove(boofySwapper, 0);
    ...
    function setBoofySwapper(address _boofySwapper) external onlyManager {
        IERC20(output).safeApprove(boofySwapper, 0);
        boofySwapper = _boofySwapper;
        if (!paused()) IERC20(output).safeApprove(boofySwapper, type(uint).max);
        emit SetBoofySwapper(_boofySwapper);
    }
}