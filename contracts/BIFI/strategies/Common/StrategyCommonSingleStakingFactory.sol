...
import "../../interfaces/boofy/IStrategyFactory.sol";
...
contract StrategyCommonSingleStakingFactory is OwnableUpgradeable, PausableUpgradeable {
    ...
    function _chargeFees(address callFeeRecipient) internal {
        IFeeConfig.FeeCategory memory fees = boofyFeeConfig().getFees(address(this));
        uint256 outputBal = IERC20(output).balanceOf(address(this));
    ...
    function _swap(address tokenFrom, address tokenTo, uint amount) internal {
        ...
        IBoofySwapper(swapper).swap(tokenFrom, tokenTo, amount);
    }
    ...
    function boofyFeeConfig() public view returns (IFeeConfig) {
        return IFeeConfig(factory.boofyFeeConfig());
    }

    function boofyFeeRecipient() public view returns (address) {
        return factory.boofyFeeRecipient();
    }

    function getAllFees() external view returns (IFeeConfig.AllFees memory) {
        return IFeeConfig.AllFees(boofyFeeConfig().getFees(address(this)), depositFee(), withdrawFee());
    }
