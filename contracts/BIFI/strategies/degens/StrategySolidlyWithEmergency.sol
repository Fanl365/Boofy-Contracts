...
contract StrategySolidlyWithEmergency is BaseAllToNativeFactoryStrat {
    ...
    function _swapNativeToWant() internal override {
        ...
        if (stable) {
            uint out0 = lpToken0 != output ? IBoofySwapper(swapper).getAmountOut(output, lpToken0, lp0Amt) : lp0Amt;
            uint out1 = lpToken1 != output ? IBoofySwapper(swapper).getAmountOut(output, lpToken1, lp1Amt) : lp1Amt;
