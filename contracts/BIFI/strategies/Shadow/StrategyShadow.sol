...
contract StrategyShadow is BaseAllToNativeFactoryStrat {
    ...
    function _claim() internal override {
        ...
        if (xShadowBalance > 0) {
            ...
            uint256 amountOut = IBoofySwapper(swapper).getAmountOut(x33, shadow, shares);
            ...
            if (amountOut > (xShadowBalance / 2)) {
                ...
                IBoofySwapper(swapper).swap(x33, native, shares);
            } else {
                ...
                IBoofySwapper(swapper).swap(shadow, native, shadowBalance);
            }
        }
    }
    ...
    function _swapNativeToWant() internal override {
        ...
        if (stable) {
            ...
            uint256 out0 = IBoofySwapper(swapper).getAmountOut(native, lpToken0, lp0Amt) * 1e18 / lp0Decimals;
            uint256 out1 = IBoofySwapper(swapper).getAmountOut(native, lpToken1, lp1Amt) * 1e18 / lp1Decimals;
        ...
        if (lpToken0 != native) {
            ...
            IBoofySwapper(swapper).swap(native, lpToken0, lp0Amt);
        }

        if (lpToken1 != native) {
            IERC20(native).forceApprove(swapper, lp1Amt);
            IBoofySwapper(swapper).swap(native, lpToken1, lp1Amt);
        }
