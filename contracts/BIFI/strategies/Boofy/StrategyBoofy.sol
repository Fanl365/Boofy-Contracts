...
import { IBoofySwapper } from "../../interfaces/boofy/IBoofySwapper.sol";
import { IBoofyRewardPool } from "../../interfaces/boofy/IBoofyRewardPool.sol";
...
contract StrategyBoofy is StratFeeManagerInitializable {
    ...
    /// @param boofyFees Amount of native sent to the boofy fee recipient
    ...
    function deposit() public whenNotPaused {
        ...
        if (wantBal > 0) {
            IBoofyRewardPool(rewardPool).stake(wantBal);
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            IBoofyRewardPool(rewardPool).withdraw(_amount - wantBal);
    ...
    function _harvest(address _callFeeRecipient) internal whenNotPaused {
        IBoofyRewardPool(rewardPool).getReward();
        _swapToNative();
    ...
    function _swapToNative() internal {
        for (uint i; i < rewards.length; ++i) {
            ...
            if (rewardBal > 0) IBoofySwapper(unirouter).swap(reward, native, rewardBal);
        }
    }
    ...
    function _chargeFees(address _callFeeRecipient) internal {
        ...
        IERC20Upgradeable(native).safeTransfer(boofyFeeRecipient, boofyFeeAmount);
    ...
    function _swapToWant() internal {
        ...
        if (nativeBal > 0) IBoofySwapper(unirouter).swap(native, want, nativeBal);
    }
    ...
    function rewardsAvailable() public view returns (uint256 unclaimedReward) {
        unclaimedReward = IBoofyRewardPool(rewardPool).earned(address(this), native);
    }
    ...
    function retireStrat() external {
        ...
        IBoofyRewardPool(rewardPool).withdraw(balanceOfPool());
