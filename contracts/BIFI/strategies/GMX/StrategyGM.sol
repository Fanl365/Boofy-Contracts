...
contract StrategyGM is StratFeeManagerInitializable {
    ...
    function _swapRewardsToNative() internal {
        for (uint i; i < rewards.length; ++i) {
            ...
            if (reward != native) {
                ...
                if (amount > 0) {
                    IBoofySwapper(unirouter).swap(reward, native, amount);
                }
            }
        }
    }
    ...
    function _mintGm() internal {
        ...
        if (long != native) IBoofySwapper(unirouter).swap(native, long, halfAmount);
        if (short != native) IBoofySwapper(unirouter).swap(native, short, nativeBal - halfAmount);
