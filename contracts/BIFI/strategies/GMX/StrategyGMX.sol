...
import "../../interfaces/gmx/IBoofyVault.sol";
...
contract StrategyGMX is StratFeeManagerInitializable {
    ...
    function _swapRewardToNative() internal {
        uint256 wantBal = IERC20(want).balanceOf(address(this));
        if (wantBal > 0) IBoofySwapper(unirouter).swap(want, native, wantBal);
    }
    ...
    function chargeFees(address callFeeRecipient) internal {
        ...
        uint256 boofyFeeAmount = feeBal * fees.boofy / DIVISOR;
    ...
    function retireStrat() external {
        ...
        IBoofyVault.StratCandidate memory candidate = IBoofyVault(vault).stratCandidate();
    ...
    function acceptTransfer() external {
        address prevStrat = IBoofyVault(vault).strategy();
