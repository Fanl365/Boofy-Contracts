...
import "./BoofyBalancerStructs.sol";
...
contract StrategyAuraGyroMainnet is StratFeeManagerInitializable {
    ...
    BoofyBalancerStructs.Input public input;
    ...
    BoofyBalancerStructs.BatchSwapStruct[] public nativeToLp0Route;
    BoofyBalancerStructs.BatchSwapStruct[] public lp0ToLp1Route;
    BoofyBalancerStructs.BatchSwapStruct[] public outputToNativeRoute;
    address[] public nativeToLp0Assets;
    address[] public lp0Tolp1Assets;
    address[] public outputToNativeAssets;

    mapping(address => BoofyBalancerStructs.Reward) public rewards;
    ...
    event ChargedFees(uint256 indexed callFees, uint256 indexed boofyFees, uint256 indexed strategistFees);

    function initialize(
        address _want,
        BoofyBalancerStructs.BatchSwapStruct[] memory _nativeToLp0Route,
        BoofyBalancerStructs.BatchSwapStruct[] memory _lp0ToLp1Route,
        BoofyBalancerStructs.BatchSwapStruct[] memory _outputToNativeRoute,
        address _booster,
        uint256 _pid,
        ...
        for (uint i; i < rewardTokens.length; ++i) {
            ...
            if (bal >= rewards[rewardTokens[i]].minAmount) {
                if (rewards[rewardTokens[i]].assets[0] != address(0)) {
                    BoofyBalancerStructs.BatchSwapStruct[] memory swapInfo = new BoofyBalancerStructs.BatchSwapStruct[](rewards[rewardTokens[i]].assets.length - 1);
    ...
    function chargeFees(address callFeeRecipient) internal {
        ...
        uint256 boofyFeeAmount = nativeBal * fees.boofy / DIVISOR;
        IERC20(native).safeTransfer(boofyFeeRecipient, boofyFeeAmount);

        uint256 strategistFeeAmount = nativeBal * fees.strategist / DIVISOR;
        IERC20(native).safeTransfer(strategist, strategistFeeAmount);

        emit ChargedFees(callFeeAmount, boofyFeeAmount, strategistFeeAmount);
    }
    ...
    function addRewardToken(address _token, BoofyBalancerStructs.BatchSwapStruct[] memory _swapInfo, address[] memory _assets, bytes calldata _routeToNative, uint _minAmount) external onlyOwner {
