...
contract StrategyRa is StratFeeManagerInitializable {
    ...
    function chargeFees(address callFeeRecipient) internal {
        ...
        uint256 boofyFeeAmount = nativeBal - callFeeAmount - strategistFeeAmount;
    ...
    function swapRewards() internal {
        ...
        if (toNative > 0) IBoofySwapper(unirouter).swap(output, native, toNative);

        for (uint i; i < rewards.length; ++i) {
            if (rewards[i] != native) {
                uint256 bal = IERC20(rewards[i]).balanceOf(address(this));
                if (bal > 0) {
                    IBoofySwapper(unirouter).swap(rewards[i], native, bal);
                }
            }
        }
    }
    ...
    function addLiquidity() internal {
        ...
        if (stable) {
            ...
            uint256 out0 = lpToken0 != native
                ? IBoofySwapper(unirouter).getAmountOut(native, lpToken0, lp0Amt) * 1e18 / lp0Decimals
                : lp0Amt;
            uint256 out1 = lpToken1 != native 
                ? IBoofySwapper(unirouter).getAmountOut(native, lpToken1, lp1Amt) * 1e18 / lp1Decimals
                : lp1Amt;
        ...
        if (lpToken0 != native) {
            IBoofySwapper(unirouter).swap(native, lpToken0, lp0Amt);
        }

        if (lpToken1 != native) {
            IBoofySwapper(unirouter).swap(native, lpToken1, lp1Amt);
        }
    ...
    function callReward() public view returns (uint256) {
        ...
        uint256 nativeOut;
        if (outputBal > 0) {
            nativeOut = IBoofySwapper(unirouter).getAmountOut(output, native, outputBal);
            }
