...
contract StrategyLynexGamma is StratFeeManagerInitializable {
    ...
    function hook(address sender, uint256 amount0, uint256, bytes memory) external {
        ...
        IBoofySwapper(unirouter).swap(output, paymentToken, outputTokenBal);
        ...
        IBoofySwapper(unirouter).swap(paymentToken, native, paymentTokenBal);
        flashOn = false;
    }
    ...
    function flashExercise(uint256 _amount) internal {
        ...
        flashOn = true;
        IFlashPool(flashPool).swap(amountNeeded, 0, address(this), "Boofy");
    }
    ...
    function addLiquidity() internal {
        ...
        if (lpToken0 != native) IBoofySwapper(unirouter).swap(native, lpToken0, toLp0);
        if (lpToken1 != native) IBoofySwapper(unirouter).swap(native, lpToken1, toLp1);
    ...
    function quoteAddLiquidity() internal view returns (uint toLp0, uint toLp1) {
        ...
        if (isFastQuote) {
            ...
            uint out0 = lp0Amt;
            uint out1 = lp1Amt;
            if (lpToken0 != native) out0 = IBoofySwapper(unirouter).getAmountOut(native, lpToken0, lp0Amt);
            if (lpToken1 != native) out1 = IBoofySwapper(unirouter).getAmountOut(native, lpToken1, lp1Amt);
