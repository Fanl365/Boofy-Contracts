...
contract StrategySwapBased is StratFeeManagerInitializable {
    ...
    function _convertRewards() internal {
        ...
        if (rewards.length != 0) {
            for (uint i; i < rewards.length; i++) {
                ...
                if (toNative > 0) IBoofySwapper(unirouter).swap(reward, native, toNative);
            }
        }
        ...
        if (optionBal > 0) {
            ...
            uint256 twapRate = IBoofySwapper(unirouter).getAmountOut(output, native, optionBal);
    ...
    function receiveFlashLoan(
        ...
        IBoofySwapper(unirouter).swap(output, native, IERC20(output).balanceOf(address(this)));
    ...
    function addLiquidity() internal {
        ...
        if (lpToken0 != native) IBoofySwapper(unirouter).swap(native, lpToken0, nativeHalf);
        if (lpToken1 != native) IBoofySwapper(unirouter).swap(native, lpToken1, nativeHalf);
