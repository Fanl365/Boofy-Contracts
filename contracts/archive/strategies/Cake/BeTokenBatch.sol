...
contract BeTokenBatch is Ownable {
    ...
    constructor(
        address _rewardPool,
        address _unirouter, 
        address _boofyFeeBatch,
        address[] memory _route
    ) {
        rewardPool = IRewardPool(_rewardPool);
        unirouter = IUniswapRouterETH(_unirouter);
        boofyFeeBatch = _boofyFeeBatch;
        route = _route;
    ...
    function harvest() public {
        ...
        if (fee > 0) {
            ...
            unirouter.swapExactTokensForTokens(feeBal, 0, route, boofyFeeBatch, block.timestamp);
        }
    ...
    function setFeeBatch(address _feeBatch) external onlyOwner {
        emit NewFeeBatch(boofyFeeBatch, _feeBatch);
        boofyFeeBatch = _feeBatch;
    }
