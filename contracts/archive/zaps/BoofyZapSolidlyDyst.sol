...
contract BoofyZapSolidlyDyst {
    ...
    function beefOut(address boofyVault, uint256 withdrawAmount) external {
        (IBoofyVaultV6 vault, ISolidlyPair pair) = _getVaultPair(boofyVault);
    ...
    function _getVaultPair(address boofyVault) private pure returns (IBoofyVaultV6 vault, ISolidlyPair pair) {
    ...
    function estimateSwap(address boofyVault, address tokenIn, uint256 fullInvestmentIn) public view returns(uint256 swapAmountIn, uint256 swapAmountOut, address swapTokenOut) {
        checkWETH();
        (, ISolidlyPair pair) = _getVaultPair(boofyVault);
