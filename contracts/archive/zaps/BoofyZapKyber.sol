...
contract BoofyZapKyber {
    ...
    function beefInETH (address boofyVault, bytes memory token0, bytes memory token1) external payable {
        require(msg.value >= minimumAmount, 'Boofy: Insignificant input amount');

        IWETH(WETH).deposit{value: msg.value}();
        _swapAndStake(boofyVault, WETH, WETH, token0, token1);
    }

    function beefIn (address boofyVault, address inputToken, uint256 tokenInAmount, bytes memory token0, bytes memory token1) public {
        require(tokenInAmount >= minimumAmount, 'Boofy: Insignificant input amount');
        require(IERC20(inputToken).allowance(msg.sender, address(this)) >= tokenInAmount, 'Boofy: Input token is not approved');

        IERC20(inputToken).safeTransferFrom(msg.sender, address(this), tokenInAmount);

        _swapAndStake(boofyVault, inputToken, inputToken, token0, token1);
    }

    function beefOut (address boofyVault, uint256 withdrawAmount) external {
        address[] memory tokens = _beefOut(boofyVault, withdrawAmount);
         _returnAssets(tokens);
    }

    function _beefOut (address boofyVault, uint256 withdrawAmount) internal returns (address[] memory tokens) {
        (IBoofyVault vault, IUniswapV2Pair pair) = _getVaultPair(boofyVault);
    ...
    function beefOutAndSwap(address boofyVault, uint256 withdrawAmount, address desiredToken, bytes memory dataToken0, bytes memory dataToken1) external {
        (IBoofyVault vault, IUniswapV2Pair pair) = _getVaultPair(boofyVault);
    ...
    function quoteStableAddLiquidityRatio(IBoofyVault boofyVault) external view returns (uint256 ratio0to1) {
            (,IUniswapV2Pair pairAddress) = _getVaultPair(address(boofyVault));
            ...
            ISolidlyRouter router = ISolidlyRouter(IStrategy(boofyVault.strategy()).unirouter());
    ...
    function quoteRemoveLiquidity(IBoofyVault boofyVault, uint256 mooTokenAmt) external view returns (uint256 amt0, uint256 amt1, address token0, address token1) {
        uint256 withdrawFee = IStrategy(boofyVault.strategy()).withdrawalFee();
        uint256 liquidity = mooTokenAmt * boofyVault.balance() / boofyVault.totalSupply();
        uint256 fee = withdrawFee > 0 ? liquidity * withdrawFee / 10000 : 0;
        liquidity = liquidity - fee;
        
        (, IUniswapV2Pair pair) = _getVaultPair(address(boofyVault));
    ...
    function _getVaultPair (address boofyVault) private pure returns (IBoofyVault vault, IUniswapV2Pair pair) {
        vault = IBoofyVault(boofyVault);
    ...
    function _swapAndStake(address boofyVault, address inputToken0, address inputToken1, bytes memory token0, bytes memory token1) private {
        (IBoofyVault vault, IUniswapV2Pair pair) = _getVaultPair(boofyVault);
