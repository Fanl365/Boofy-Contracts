...
contract BoofyVelodromeV2Zap {
    ...
    constructor(address _router, address _WETH) {
        ...
        require(WETH == router.weth(), 'Boofy: WETH address not matching Router.weth()');
    }
    ...
    function beefOut (address boofyVault, uint256 withdrawAmount) external {
        (IBoofyVault vault, IPool pair) = _getVaultPair(boofyVault);
    ...
    function beefOutAndSwap(address boofyVault, uint256 withdrawAmount, address desiredToken, uint256 desiredTokenOutMin) external {
        ...
        require(token0 == desiredToken || token1 == desiredToken, 'Boofy: desired token not present in liquidity pair');
    ...
    function _getVaultPair (address boofyVault) private pure returns (IBoofyVault vault, IPool pair) {
    ...
    function _swapAndStake(address boofyVault, uint256 tokenAmountOutMin, address tokenIn) private {
        (IBoofyVault vault, IPool pair) = _getVaultPair(boofyVault);
        require(pair.factory() == router.defaultFactory(), 'Boofy: Incompatible liquidity pair'); // router.addLiquidity adds to pair from router.defaultFactory()

        (uint256 reserveA, uint256 reserveB,) = pair.getReserves();
        require(reserveA > minimumAmount && reserveB > minimumAmount, 'Boofy: Liquidity pair reserves too low');

        bool isInputA = pair.token0() == tokenIn;
        require(isInputA || pair.token1() == tokenIn, 'Boofy: Input token not present in liquidity pair');
    ...
    function estimateSwap(address boofyVault, address tokenIn, uint256 fullInvestmentIn) public view returns (uint256 swapAmountIn, uint256 swapAmountOut, address swapTokenOut) {
        (, IPool pair) = _getVaultPair(boofyVault);
        require(pair.factory() == router.defaultFactory(), 'Boofy: Incompatible liquidity pair'); // router.addLiquidity adds to pair from router.defaultFactory()
