...
// Built to safely flash loan amount needed to upgrade Boofy Vault to new strategy 
contract Rescuer is Ownable {
    ...
    IBoofyVault public boofyVault = IBoofyVault(0x42ECfA11Db08FB3Bb0AAf722857be56FA8E57Dc0);
    ...
    function tusdNeeded() public view returns (uint256) {
        return boofyVault.balance() - tusd.balanceOf(address(iToken));
    }
    ...
    function upgrade() external onlyOwner {
        ...
        IStrategy(boofyVault.strategy()).updateBalance();
        ...
        balancerVault.flashLoan(address(this), tokens, amounts, "BoofyBalancerFlash");
    }
    ...
    function receiveFlashLoan(address[] memory, uint256[] memory amounts, uint256[] memory feeAmounts, bytes memory) external {
        tusdPair.swap(0, tusdNeeded() - amounts[0], address(this), "BoofyRescue");
        uint256 balancerDebt = amounts[0] + feeAmounts[0];
        tusd.safeTransfer(address(balancerVault), balancerDebt);
    }

    function transferBoofyVaultOwnership(address _newOwner) external onlyOwner { 
        emit VaultOwnershipChanged(boofyVault.owner(), _newOwner);
        boofyVault.transferOwnership(_newOwner);
    }
