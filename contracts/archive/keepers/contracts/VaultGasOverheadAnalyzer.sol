...
import "../interfaces/IBoofyStrategyEthCall.sol";
...
contract VaultGasOverheadAnalyzer is ManageableUpgradeable, KeeperCompatibleInterface {
    ...
    IBoofyRegistry private _vaultRegistry;
        ...
        )
    {
        ...
        IBoofyVault vaultToAnalyze = IBoofyVault(vaults[currentIndex]);
    ...
    function _analyzeHarvest(IBoofyVault vault) internal view returns (bool didHarvest_, uint256 gasOverhead_) {
        IBoofyStrategyEthCall strategy = IBoofyStrategyEthCall(address(IBoofyVault(vault).strategy()));
    ...
    function _tryHarvest(IBoofyStrategyEthCall strategy) internal view returns (bool didHarvest_, uint256 gasOverhead_) {
    ...
    function _tryOldHarvest(IBoofyStrategyEthCall strategy) internal view returns (bool didHarvest_, uint256 gasOverhead_) {
        uint256 gasBefore = gasleft();
