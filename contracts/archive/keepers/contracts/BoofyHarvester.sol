...
import "../interfaces/IBoofyVault.sol";
import "../interfaces/IBoofyStrategy.sol";
import "../interfaces/IBoofyRegistry.sol";
import "../interfaces/IBoofyHarvester.sol";
import "../interfaces/IUpkeepRefunder.sol";

import "../libraries/UpkeepLibrary.sol";

contract BoofyHarvester is ManageableUpgradeable, IBoofyHarvester {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    // Contracts.
    IBoofyRegistry public _vaultRegistry;
    ...
    uint256 public _vaultHarvestFunctionGasOverhead; // Estimated average gas cost of calling harvest(). TODO: this needs to live in BoofyRegistry, and needs to be a `per vault` number.
    ...
    function initialize(
        ...
        _vaultRegistry = IBoofyRegistry(vaultRegistry_);
        ...
        )
    {
        ...
        if (numberOfVaultsToHarvest == 0) return (false, bytes("BoofyAutoHarvester: No vaults to harvest"));
    ...
    function _canHarvestVault(address vaultAddress_) internal view returns (bool canHarvest_) {
        IBoofyVault vault = IBoofyVault(vaultAddress_);
        IBoofyStrategy strategy = IBoofyStrategy(vault.strategy());
        ...
        )
    {
        ...
        uint256 vaultHarvestGasOverhead = _estimateSingleVaultHarvestGasOverhead(_vaultHarvestFunctionGasOverhead); // TODO: Pull this number from BoofyRegistry.
    ...
    function _harvestVault(address vault_) internal returns (bool didHarvest_, uint256 callRewards_) {
        IBoofyStrategy strategy = IBoofyStrategy(IBoofyVault(vault_).strategy());
