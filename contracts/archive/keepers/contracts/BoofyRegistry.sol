...
contract BoofyRegistry is ManageableUpgradeable, IBoofyRegistry {
    ...
    function _addVault(address _vaultAddress) internal {
        ...
        IBoofyVault vault = IBoofyVault(_vaultAddress);
        IBoofyStrategy strat = _validateVault(vault);
    ...
    function _validateVault(IBoofyVault _vault) internal view returns (IBoofyStrategy strategy) {
        address vaultAddress = address(_vault);

        try _vault.strategy() returns (IBoofyStrategy _strategy) {
            require(IBoofyStrategy(_strategy).vault() == vaultAddress, "Vault/Strat Mismatch");
            return IBoofyStrategy(_strategy);
        } catch {
            require(false, "Address not a Vault");
        }
    }

    function _collectTokenData(IBoofyStrategy _strategy) internal view returns (address[] memory tokens) {
        ...
        )
    {
        ...
        name_ = vault.name();
        strategy_ = IBoofyStrategy(vault.strategy());
    ...
    function getStakedVaultsForAddress(address _address) external view returns (VaultInfo[] memory stakedVaults) {
        ...
        for (uint256 vid; vid < _vaultSet.length(); vid++) {
            if (IBoofyVault(_vaultSet.at(vid)).balanceOf(_address) > 0) {
                numResults++;
            }
        }
