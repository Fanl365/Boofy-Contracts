...
/// @title Boofy fee batch
/// @author kexley, Beefy
/// @notice All Boofy fees will flow through to the treasury and the reward pool
/// @dev Wrapped ETH will build up on this contract and will be swapped via the Boofy Swapper to
/// the pre-specified tokens and distributed to the treasury and reward pool
contract BoofyFeeBatchV4 is OwnableUpgradeable {
    ...
    function _distributeTreasuryFee() private {
        ...
        for (uint i; i < treasuryTokens.tokens.length; ++i) {
            ...
            if (token != address(native)) {
                amount = IBoofySwapper(swapper).swap(address(native), token, amount);
                if (amount == 0) continue;
            }
    ...
    function _notifyRewardPool() private {
        ...
        for (uint i; i < rewardTokens.tokens.length; ++i) {
            ...
            IBoofyRewardPool(rewardPool).notifyRewardAmount(token, amount, duration);
