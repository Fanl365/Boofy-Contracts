...
task("test-data:addressbook:boofy", "Fetch boofy addressbook to inject platform addresses in forge tests")
  ...
  .setAction(async (taskArgs: { chain: keyof typeof addressBook}, hre, runSuper) => {
    ...
    const { boofyfinance } = addressBook[taskArgs.chain].platforms;
    const data = {
      keeper: boofyfinance.keeper,
      strategyOwner: boofyfinance.strategyOwner,
      vaultOwner: boofyfinance.vaultOwner,
      boofySwapper: boofyfinance.boofySwapper,
    };
